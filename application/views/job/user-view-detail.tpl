...
{/if}
        ...
        <div class="mb-2">
        <a href="/index.php/user/jobs/1"><i class="icofont-rounded-left"></i>&nbsp; Back to Job Listing</a>
        </div>
          <div class="boxStyle p-4">
                <div class="row">
                    ...
                    {include file="../common/projectleftmenu.tpl"}
                        ...
                        {if isset($logged_in_user.UserRole) &&  $currtimesheet.UserId===$logged_in_user.UserId}
  ...
  <input type="text" id="startdate{$currtimesheet["Id"]}" placeholder="Time" class="form-control"   name="txtStartTime"  value="{$currtimesheet["StartTime"]}" >
                        ...
                        {if isset($logged_in_user.UserRole) &&  $currjobresouce.UserId===$logged_in_user.UserId}
                        <div class="form-group row">
                            <div class="col-sm-4">
...
var timepicker = new TimePicker([{if count($currtimesheets)>0}
    {foreach $currtimesheets as $currtimesheetId}{if isset($logged_in_user.UserRole) &&  $currtimesheetId.UserId===$logged_in_user.UserId}'startdate{$currtimesheetId.Id}','enddate{$currtimesheetId.Id}',{/if}{/foreach}
{/if}{if count($currjobresouces)>0}
    {foreach $currjobresouces as $currjobresouceId}{if isset($logged_in_user.UserRole) &&  $currjobresouceId.UserId===$logged_in_user.UserId}'resstartdate{$currjobresouceId.ResourceId}','resenddate{$currjobresouceId.ResourceId}',{/if}{/foreach}
{/if}], {
  theme: 'dark',
  lang: 'en'
});
...
$.ajax({
            ...
            success: function(result){
                if(result=="Sorry cannot log old dates.")
                    ...
                    location.href ='{$host_url}/index.php/user-view-job-detail/'+{$currjobs_list.Id};
            },
            error: function(data){
                console.log('error');
            }
